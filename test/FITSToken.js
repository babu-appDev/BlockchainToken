...
contract('FITSToken', function(accounts) {
  var tokenInstance;

  it('initializes the contract with the correct values', function() {
    return FITSToken.deployed().then(function(instance){
      tokenInstance = instance;
      return tokenInstance.name();
    }).then(function(name){
      assert.equal(name, 'FITS SHARE', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol){
      assert.equal(symbol, 'FITS', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard){
      assert.equal(standard, 'FITS Token v1.0', 'has the correct standard');
    });
  });
  ...
  it('Allocates the initial supply upon deployment', function() {
    ...
    }).then(function(admminBalance) {
      assert.equal(admminBalance.toNumber(), 1000000, 'Allocates the initial supply to the admin account')
    });
  });
  ...
  it('handles delegated token transfers', function() {
    return FITSToken.deployed().then(function(instance) {
      ...
      // Try transfering a value greater than the approved amount
